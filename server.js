//NPM Packages required for YAML to JSON and JSON to YAML conversion. Used to fulfill bonus requirements.
const yaml2json = require('js-yaml')
const json2yaml = require('json2yaml')

//Node package required to read/write from external file. Used to read/write to database file.
const fs = require('fs')

//Variable declarations.
const inputFile = 'database.yml'
let database = {
  users: {},
  articles: {},
  comments: {},
...
//Defines the REST routes and methods for the server
const routes = {
  '/users': {
...
//Function used to load data into database object. Returns converted YAML string as Javascript object
function loadDatabase(){
	obj = yaml2json.load(fs.readFileSync(inputFile, {encoding: 'utf-8'}));
	return(obj);
}

//Function used to save database object to a separate file. Converts Javascript object to a YAML string and writes to inputFile
function saveDatabase(){
	databaseYaml = json2yaml.stringify(database)
	fs.writeFile(inputFile, databaseYaml, function(err) {
		if(err) {
...
//Function used to create comments. Accepts string and object variables, returns an object. 
function createComment(url, request){
	const response = {};
	const requestComment = request.body && request.body.comment;
	//Error handling for improper request. Invalid request, username,message content or article Id.
	if (requestComment && requestComment.body && requestComment.username && database.users[requestComment.username] && requestComment.articleId && database.articles[requestComment.articleId]){
		...
		//Adds comment data to the database object
...
//Function used to update comments. Accepts string and object variables, returns an object
function updateComment(url, request){
	const id = Number(url.split('/').filter(segment => segment)[1]);
	const savedComment = database.comments[id];
	const requestComment = request.body && request.body.comment;
	const response = {};
	//Error handling for improper request. Invalid comment Id or request. 
...
//Function used to delete comments. Accepts string and object variables, returns an object.
function deleteComment(url, request){
	const id = Number(url.split('/').filter(segment => segment)[1]);
	const savedComment = database.comments[id];
	const response = {};
	//Error handling for improper request. Invalid comment Id
	if (savedComment) {
		//Removes and cleans up data in database object 
...
//Function used to upvote comments. Accepts strings and object variables, returns an object
function upvoteComment(url, request) {
	const id = Number(url.split('/').filter(segment => segment)[1]);
	const username = request.body && request.body.username;
	let savedComment = database.comments[id];
	const response = {};
	//Error handling for improper request. Invalid comment or user
...
//Function used to downvote comments. Accepts strings and object variables, returns an object
